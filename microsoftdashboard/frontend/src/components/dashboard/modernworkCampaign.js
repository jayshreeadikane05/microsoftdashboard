...
const ModernworkCampaign = () => {
  ...
  const fetchCampaigns = async (page = currentPage, limit = itemsPerPage) => {
    ...
    try {
      const token = Cookies.get('token');
      const response = await axios.post(`${BASE_API_URL}/pri/campagin/modernwork`, {
        year: searchYear,
...
export default ModernworkCampaign;
